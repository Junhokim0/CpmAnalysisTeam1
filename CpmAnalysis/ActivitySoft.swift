...
class Project {
    ...
    private func printProject(activities: [ActivitySoft], criticalActivities: [ActivitySoft]) {
        ...
        result.append ("Total Duration: \(calculateTotalDuration (activities: activities))")
    }
    ...
    public func scheduleCalculation (startDate: Int32) {
        for schedule in schedules {
            ...
            let totalDuration = calculateTotalDuration (activities: schedule.schedule)
        }
    }
    
    public func calculateTotalDuration(activities: [ActivitySoft]) -> Int32 {
        let maxEarlyFinish = activities.map { $0.earlyFinish }.max () ?? 0
        let minEarlyStart = activities.map { $0.earlyStart }.min() ?? 0
        let totalDuration = maxEarlyFinish - minEarlyStart + 1
